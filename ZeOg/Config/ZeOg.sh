...
echo "                ${purple}+------------------------------------------------------------+"
echo                                                                              
echo IMPORTANT: Be sure that ZeOg is on the desktop.
echo "${normal}[1]. Use/info default scripts."
echo [2]. Download third party scripts.
echo '[3]. About.'
echo [4]. Help.
echo [5]. Update.
echo [6]. Restart ZeOg.
...
then
  echo "${lightgreen}[+]Launching shell...."
  exec /root/Desktop/ZeOg/Config/Downloaded-scripts/download-list.sh
fi
if [ $choice1 == 3 ]
then
  echo "${lightgreen}[+]Launching shell...."
  exec /root/Desktop/ZeOg/Config/about.sh 
fi
if [ $choice1 == 4 ]
then 
  echo "${lightgreen}[+]Launching shell...."
  exec /root/Desktop/ZeOg/Config/help.sh
fi
if [ $choice1 == 5 ]
then 
  echo "${bold}[+]Launching Update...."
  echo "${bold}[+]The Update will be saved at download!"
  exec /root/Desktop/ZeOg/Config/update.sh
fi
if [ $choice1 == 6 ]
then
  exec 
  /root/Desktop/ZeOg/Config/ZeOg.sh
fi
